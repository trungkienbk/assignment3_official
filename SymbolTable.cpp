    ...
    while (!f.eof()){
       // cout<<"Kien dep trai"<<endl;
        ...
        if(regex_match(ins,linear)){
            if(count_linear > 1) throw InvalidInstruction(ins);
            SetLinear(ins);
            count_linear++;
        }
...
void SymbolTable::call_func(string ins,int cur_level) {
    ...
    ins_tmp+=value_func;
   if(isKeyword(ins_tmp)) throw InvalidInstruction(ins);
...
void SymbolTable::assign_variable(string ins,int cur_level) {
  ...
  //  if(check_val.argList != "") throw TypeMismatch(ins);//// Code cu
    if(check_val.argList != "") throw InvalidInstruction(ins);//// Code cu
                ...
                if(temp[0]>='0'  &&temp[0]<='9'){
                    ...
                    if(temp_argu.type =="")  throw TypeCannotBeInferred(ins);
                        ...
                        if(temp_argu.type == "string"){
                            newHash.arr[idx_func_real].argList[k]= 's';
                        } else newHash.arr[idx_func_real].argList[k] = 'n';
                    }
                }++k;
            }
        }
...
void SymbolTable::assign_function(string ins, int cur_level) {
    ...
    if(newHash.arr[idx_func].type == "void") throw TypeMismatch(ins);
